...
CVA (BBB proxy) & \$210,396 & Price of counterparty default risk, unilateral, uncollateralized (Section 8) \\
...
\item Mean reversion: a = 0.0167 for USD from the swaption cube; JPY reuses the USD value because real JPY volatility rises with tenor (two independent sources).
...
\rowcolor{gray!9}sigma & 0.276\%/yr & REALISED TONA vol, 3y window: real Bank of Japan data (replaces an earlier swaption-implied 0.400\%) \\
a (mean reversion) & 0.0167 & Fallback to the USD value: both real JPY calibrations gave a negative a (Section 6.2) \\
...
Wired into simulate\_paths()? & NO (disclosed) & JPY equity/FX drift still uses r\_USD minus the constant differential (2.64\%, from real Bloomberg curves) \\
\bottomrule
\end{longtable}}
...
Current measured cost for the 3,000-scenario reporting run: 2 s simulation of paths, 1239 s repricing (413 ms/scenario, including the extra MPOR look-ahead nodes).
...
\caption{Left: USD SOFR-futures fit. Right: fitted a for each route. USD routes give sensible positive values; both JPY routes give a NEGATIVE a.}
\end{figure}
{\footnotesize\setlength{\LTpre}{4pt}\setlength{\LTpost}{6pt}
...
\caption{Real realised JGB yield vol by tenor. At every window tried (1y, 3y, 10y) long tenors are MORE volatile than short ones, the reverse of the decay Hull-White assumes.}
\end{figure}
This is not bad data; it is a structural finding confirmed by two independent real sources (a 2026 swaption cube and 52 years of JGB yields). The plausible reason: for two decades the Bank of Japan pinned the SHORT end (zero and negative policy rates, yield-curve control) so short-tenor vol was suppressed while longer tenors moved more freely. A single-factor model with positive mean reversion cannot represent that shape, and a negative a would make the short rate diverge. \textbf{Choice:} reuse the USD value a = 0.0167 for the JPY factor and label it as a fallback. What would truly fix it is a two-factor or regime-dependent model, or a long history of the full JPY swap curve; this is listed under future work.
...
\endhead
CPTY\_A & 5,403 & 3\% \\
\rowcolor{gray!9}CPTY\_B & 3,702 & 2\% \\
CPTY\_C & 201,291 & 96\% \\
\rowcolor{gray!9}Portfolio & 210,396 & 100\% \\
\bottomrule
\end{longtable}}
...
\endhead
AAA & 89,023 & 0.42x \\
\rowcolor{gray!9}AA & 125,895 & 0.60x \\
A & 143,238 & 0.68x \\
\rowcolor{gray!9}BBB & 210,396 & 1.00x \\
BB & 325,034 & 1.54x \\
\rowcolor{gray!9}B & 588,063 & 2.80x \\
\bottomrule
\end{longtable}}
...
ccs delta & 1,729,848 \\
\rowcolor{gray!9}eq delta & 105,515 \\
eq vega & 6,637 \\
\rowcolor{gray!9}Total SA-CVA capital requirement (m\_CVA = 1) & 1,927,192 \\
Risk-weighted assets (12.5 x capital) & 24,089,897 \\
\rowcolor{gray!9}Total if the pre-2023 multiplier m\_CVA = 1.25 applied & 2,408,990 \\
\bottomrule
\end{longtable}}
...
\rowcolor{gray!9}8 & JPY mean-reversion fit gave a negative a & Diagnosed as a real structural property (two sources); guarded with documented fallback \\
\bottomrule
\end{longtable}}
...
\item \textbf{JPY:} mean reversion is a fallback and the JPY rate does not yet drive JPY equity/FX drift (two trades).
...
\begin{itemize}
\item The engine is validated (t=0 self-check, martingale, VaR benchmark, stress test, 69 tests) and reproducible with a single command per stage.
...
\rowcolor{gray!9}USD swaption vol cube; JPY OIS curve and swaption vols; USDJPY forward points & Bloomberg one-time export, snapshot 2026-08-31 & Provided by the project team & USD mean reversion; JPY factor; JPY-USD rate differential; FX forwards & Licensed: derived numbers only in the report \\
...
\rowcolor{gray!9}SA-CVA parameters and formulas & BIS, Basel Framework MAR50 (July 2020 revisions) & Public PDF & Risk weights, correlations, aggregation & Public \\
Equity sector, country, market cap & yfinance & Public API & SA-CVA equity buckets & Live \\
\bottomrule
\end{longtable}}

Assumptions that are not sourced from data: counterparty ratings (BBB), LGD (60\%), MPOR (10 business days), the uncollateralized status of the book, the correlation structure being static, and the JPY mean-reversion fallback to the USD value.
...
Mean reversion & Regression of ln(vol) on tenor (vol decay); swaption cube preferred, futures and JGB as checks & USD 0.0167; futures 0.0458; JPY invalid, fallback to USD & models/hw\_calibration.py \\
\rowcolor{gray!9}JPY factor & Second Hull-White factor on the real JPY OIS curve; realised TONA volatility & sigma 0.276\%; USD-JPY rate correlation -0.04 (not significant) & models/calibration.py \\
Equities and FX & Correlated geometric Brownian motion, exact log step, drift r-q & 3y realised vols; JPY names via r\_USD minus differential (2.64\%) & models/equity\_fx.py \\
...
\rowcolor{gray!9}tests/ & 69 tests: engine, time grid, MPOR, negative rates, factor model, BOJ and MOF data, tail convergence, median PFE, CVA and SA-CVA \\
\bottomrule
\end{longtable}}
